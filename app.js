    ...
    function renderTable() {
        ...
        fundsData.forEach((fund, index) => {
            ...
            const diffFormatted = diff > 0 
                ? `<span class="amount-plus">+¥${diff.toLocaleString()}</span>` 
                : (diff < 0 ? `<span class="amount-minus">-¥${Math.abs(diff).toLocaleString()}</span>` : `±¥0`);
            ...
            tr.innerHTML = `
                ...
                <td class="text-center">
                    <input type="number" min="0" max="100" step="0.1" data-index="${index}" value="${fund.targetPct.toFixed(1)}">
                </td>
